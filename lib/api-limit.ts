...
export const increaseApiLimit = async () => {
    ...
    const response: UserApiLimit[] = await dbClient.select().from(userApiLimit)
    .where(eq(userApiLimit.userId, userId))

    const userLimit = response[0]
    if(userLimit) {
        await dbClient.update(userApiLimit)
...
export const checkApiLimit = async () => {
    ...
    const response: {count: number}[] = await dbClient.select({
